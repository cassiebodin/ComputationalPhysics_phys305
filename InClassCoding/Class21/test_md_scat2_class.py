...
#  add code to do hard scattering in the center of mass here
#  follow the prescription given in the class notes
#  put the new velocities in the vx and vy arrays
      for i in range(nballs-1):
            for j in range(i+1, nballs):
                  if ((x[i] -x[j])**2 + (y[i]-y[j])**2) <eps2:
                        ...
                        u1x= vx[i] - vx_cm
                        u1y= vy[i] - vy_cm
                        umag = np.sqrt(u1x**2+u1y**2)

                        r = random.random()*2*np.pi

                        vx[i]= umag*(np.cos(r)) +vx_cm
                        vx[j]= umag*(-np.cos(r)) +vx_cm
                        vy[i]= umag*( np.sin(r)) +vy_cm
                        vy[j]= umag* (-np.sin(r)) +vy_cm
